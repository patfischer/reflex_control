...
#include <reflex_control/cartesian_velocity_controller.h>
    ...
    for (size_t i = 0; i < q_start.size(); i++) {
      if (std::abs(state_handle.getRobotState().q_d[i] - q_start[i]) > 0.1) {
        ROS_ERROR_STREAM(
            ...
            "for running this example. Run `roslaunch reflex_control "
...
PLUGINLIB_EXPORT_CLASS(reflex_control::CartesianVelocityController,
